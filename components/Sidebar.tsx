...
const dataToolLinks: NavLink[] = [
  ...
  { href: '/api/accounts/export', label: 'Download Accounts Export', icon: Download },
  { href: '/api/ptps/report/export', label: 'Download PTP Report', icon: Download },
];
...
export function Sidebar() {
  ...
  const { links, recoveryLinks, dataLinks } = useMemo(() => linksForRole(role), [role]);
  ...
  const activeHref = useMemo(() => {
    const candidates = [...links, ...reportSubLinks, ...recoveryLinks, ...dataLinks].sort(
  ...
  }, [pathname, links, recoveryLinks, dataLinks]);
  ...
  const isDataSectionActive = dataLinks.some(
    (link) => pathname === link.href || pathname.startsWith(`${link.href}/`)
  );
            ...
            {dataLinks.length > 0 ? (
              <div className="pt-2">
                <button
                  ...
                  title={effectiveCollapsed ? 'Uploads & Downloads' : undefined}
                >
                  <Upload size={18} />
                  {!effectiveCollapsed ? (
                    <>
                      <span className="flex-1 truncate">Uploads & Downloads</span>
                      <ChevronDown
                        size={16}
                        className={`transition-transform ${dataToolsOpen ? 'rotate-180' : 'rotate-0'}`}
                      />
                    </>
                  ) : null}
                </button>

                {!effectiveCollapsed && dataToolsOpen ? (
                  <div className="mt-1 space-y-1 pl-4">
